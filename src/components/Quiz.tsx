  ...
  const next = () => {
    if (current + 1 >= questions.length) {
      ...
      questions.forEach((qq, i) => {
        const c = catStats[qq.category] || { correct: 0, total: 0 };
        if (i < current || i === current) c.total++;
        if (i < current) {
          // already counted in handleSelect - approximate
        }
        c.total = questions.length;
      });
    ...
    return (
      <div className="p-4 max-w-lg mx-auto">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => setMode('menu')} className="px-6 py-3 rounded-lg font-semibold" style={{ background: '#8e44ad' }}>Back to Menu</button>
          <button onClick={() => startQuiz(selectedCat)} className="px-6 py-3 rounded-lg font-semibold border border-[#8e44ad]">Retry</button>
        </div>
      </div>
    );
  }
