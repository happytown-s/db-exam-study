...
export default function SubjectBTraining() {
  ...
  if (mode === 'results') {
    ...
    return (
      <div className="p-4 max-w-lg mx-auto text-center">
        ...
        <button onClick={() => setMode('menu')} className="px-6 py-3 rounded-lg font-semibold" style={{ background: '#8e44ad' }}>Back</button>
      </div>
    );
  }
      ...
      <div className="p-4 max-w-lg mx-auto">
        ...
        <div className="space-y-3">
          {q.options.map((opt, i) => {
            ...
            if (showAnswer) {
              ...
              cls += ' border-[#1a1a3e] hover:border-[#9b59b6]';
            }
            return (
        ...
        {showAnswer && (
          <div className="mt-6 space-y-3">
            ...
            {q.sqlCode && (
              <div className="p-4 rounded-lg" style={{ background: '#1a1a3e' }}>
                <p className="text-sm font-semibold mb-2" style={{ color: '#9b59b6' }}>SQL:</p>
                <pre className="text-sm text-[#a0a0c0] overflow-x-auto whitespace-pre-wrap">{q.sqlCode}</pre>
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <p className="text-sm text-[#a0a0c0] mb-4">Schema design, query writing, performance tuning, transactions</p>
      <button onClick={() => startQuiz('all')} className="w-full text-left p-4 rounded-lg border border-[#1a1a3e] hover:border-[#9b59b6] transition-all mb-4">
