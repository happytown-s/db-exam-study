...
import examData from '../data/db-exam.json';
...
export default function Progress() {
  ...
  useEffect(() => {
    try { setQuizStats(JSON.parse(localStorage.getItem('db-quiz-stats') || '{}')); } catch {}
    try { setCalcStats(JSON.parse(localStorage.getItem('db-calc-stats') || '{}')); } catch {}
    try { setBStats(JSON.parse(localStorage.getItem('db-b-stats') || '{}')); } catch {}
    try { setWrongIds(JSON.parse(localStorage.getItem('db-quiz-wrong') || '[]')); } catch {}
  }, []);
    ...
    <div className="p-4 max-w-lg mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="p-3 rounded-lg text-center" style={{ background: '#1a1a3e' }}>
          <p className="text-2xl font-bold" style={{ color: '#9b59b6' }}>{(examData as Question[]).length}</p>
          <p className="text-xs text-[#a0a0c0]">Exam Qs</p>
        </div>
        <div className="p-3 rounded-lg text-center" style={{ background: '#1a1a3e' }}>
          <p className="text-2xl font-bold" style={{ color: '#9b59b6' }}>{(calcData as Question[]).length}</p>
          <p className="text-xs text-[#a0a0c0]">Calc Qs</p>
        </div>
        <div className="p-3 rounded-lg text-center" style={{ background: '#1a1a3e' }}>
          <p className="text-2xl font-bold" style={{ color: '#9b59b6' }}>{wrongIds.length}</p>
      ...
      <button
        onClick={() => {
          if (confirm('Clear all progress data?')) {
            localStorage.removeItem('db-quiz-stats');
            localStorage.removeItem('db-calc-stats');
            localStorage.removeItem('db-b-stats');
            localStorage.removeItem('db-quiz-wrong');
            setQuizStats({});
            setCalcStats({});
            setBStats({});
            setWrongIds([]);
